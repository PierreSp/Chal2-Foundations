...
def initBucketLat(lat: Column) : Column = {
    val BucketLat = floor((lat + lit(180))*lit(177))
    return BucketLat
}

def initBucketLong(long: Column) : Column = {   
    val BucketLong = floor((long + lit(180))*lit(177))
    return BucketLong
}
...
withColumn("Pickup_Long_Bucket", explode(array($"Pickup_Long_Bucket"-1, $"Pickup_Long_Bucket", $"Pickup_Long_Bucket"+1))).unionAll(tripsBuckets.withColumn("Pickup_Lat_Bucket", explode(array($"Pickup_Lat_Bucket"-1, $"Pickup_Lat_Bucket", $"Pickup_Lat_Bucket"+1)))).unionAll(tripsBuckets.withColumn("Pickup_Time_Bucket", explode(array($"Pickup_Time_Bucket", $"Pickup_Time_Bucket"-1))))


//Join with trips where buckets of pickup and dropoff are the same
val JoinedTrips = tripsClones.as("to").
join(tripsBuckets.as("back"), $"to.Pickup_Long_Bucket" === $"back.Dropoff_Long_Bucket" && $"to.Pickup_Lat_Bucket" === $"back.Dropoff_Lat_Bucket" && $"to.Dropoff_Time_Bucket" === $"back.Pickup_Time_Bucket" && $"back.Pickup_Long_Bucket" === $"to.Dropoff_Long_Bucket" && $"back.Pickup_Lat_Bucket" === $"to.Dropoff_Lat_Bucket", "inner")

val Joinedfilter = JoinedTrips.drop($"to.Pickup_Long_Bucket").drop($"to.Pickup_Lat_Bucket").drop($"to.Dropoff_Long_Bucket").drop($"to.Dropoff_Lat_Bucket").drop($"to.Pickup_Time_Bucket").drop($"to.Dropoff_Time_Bucket").drop($"back.Pickup_Long_Bucket").drop($"back.Pickup_Lat_Bucket").drop($"back.Dropoff_Long_Bucket").drop($"back.Dropoff_Lat_Bucket").drop($"back.Pickup_Time_Bucket").drop($"back.Dropoff_Time_Bucket").drop($"back.tpep_dropoff_datetime").drop($"to.tpep_pickup_datetime")
...
val exactJoinedTripsTime2 = JoinedUnique.where((($"to.tpep_dropoff_datetime").cast("long")) + 28800D > (($"back.tpep_pickup_datetime").cast("long")))
